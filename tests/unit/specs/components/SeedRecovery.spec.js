...
describe('SeedRecovery', () => {
  ...
  describe('bahavior', () => {
    ...
    it('should close password modal', async () => {
      ...
      await wrapper.vm.$nextTick();
      expect(wrapper.find('password-modal-stub').exists()).toBe(true);
      wrapper.find('password-modal-stub').vm.$emit('cancel');
      await wrapper.vm.$nextTick();
      expect(wrapper.find('password-modal-stub').exists()).toBe(false);
    });
    ...
    it('should emit lock if recovered seed is empty', async () => {
      expect.assertions(2);
      accountsModule.actions.recoverSeed.mockResolvedValueOnce(null);
      wrapper.setData({
        isPasswordModalVisible: true,
      });
      await wrapper.vm.$nextTick();
