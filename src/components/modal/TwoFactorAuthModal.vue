<template lang="html">
  <v-modal @close="close">
    <template slot="header">Two Factor Authentication</template>
    <p class="lead">To protect your account, we strongly recommend ebaling
      two factor authentication. Once enabled, you will be able to log in by
      entering a code generated by an authenticator app.</p>

    <p class="lead">Scan the QR code below into <strong>Google
        Authenticator</strong>. If you are unable to scan the code, you can
      also enter the secret manually.</p>

    <v-form id="twoFactorAuth" @submit="confirm">
      <div class="field" v-if="secret && email">
        <div class="has-text-centered">
          <img :src="qrCodeSrc" />
        </div>
        <p class="lead">Write down the recovery secret below:</p>
        <label class="label">Secret</label>
        {{secret}}
      </div>
      <p class="lead">Enter the 6 digit code generated by your
        authenticator app.</p>
      <v-input v-model="code"
               type="number"
               size="6"
               label="Verification Code"
               name="verificationCode"
               validator="required|digits:6"/>
    </v-form>
    <div class="buttons" slot="footer">
      <v-button form="twoFactorAuth"
                className="is-primary is-medium"
                :loading="isLoading">Verify</v-button>
    </div>
  </v-modal>
</template>

<script>
import VModal from '@/components/ui/VModal';
import VForm from '@/components/ui/form/VForm';
import VInput from '@/components/ui/form/VInput';
import VButton from '@/components/ui/form/VButton';

export default {
  name: 'two-factor-auth-modal',
  data() {
    return {
      code: null,
    };
  },
  computed: {
    qrCodeSrc() {
      const otpAuthUri = `otpauth://totp/Endpass:${this.email}?secret=${
        this.secret
      }`;

      return `https://chart.googleapis.com/chart?chs=166x166&chld=L|0&cht=qr&chl=${otpAuthUri}`;
    },
  },
  props: {
    secret: String,
    email: String,
    isLoading: {
      type: Boolean,
      default: false,
    },
  },
  methods: {
    confirm() {
      this.$emit('confirm', this.code);
    },
    close() {
      this.$emit('close');
    },
  },
  components: {
    VModal,
    VForm,
    VInput,
    VButton,
  },
};
</script>
